    ...
    public void onAuthenticationSuccess(HttpServletRequest request,
        ...
        if (existsUser.isPresent()) {
            ...
            */
            ...
            // todo: Burası refactor edilebilir.
             boolean isUpdateNeeded = true;
            for (Provider item : userDetails.getProviders()) {
                    if (item.getProviderName().equals(provider)) {
                        // daha önce bu provider ile login olmuş.
        ...
        var authToken = jwtService.generateToken(mappedContact.getEmail(), secret, tokenValidity);
        JwtDto jwtDto = new JwtDto(authToken);
        response.setContentType("application/json");
        new ObjectMapper().writeValue(response.getWriter(), jwtDto);
    }

    //arslan.kucukkafa: Farklı bir oauth provider ile login olmuş bir kullanıcı, daha önce login olmuşsa, db yeni propertyler ile güncellenmeli.
